...
use Fluid\LiquidORM\DataMapper\Exception\DataMapperInvalidArgumentException;
...
class DataMapperEnvironmentConfiguration
{
    ...
    /**
     * Get the user defined DB connection array
     *
     * @param string $driver
     *
     * @return array
    ...
    private function isCredentialsValid(string $driver) : void
    {
        ...
        if (!in_array($driver, array_keys($this->credentials[$driver]))) {
            throw new DataMapperInvalidArgumentException('Invalid or unsupported DB\'s driver.');
        }
    }
}
